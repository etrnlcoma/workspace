...
class BDControllerCfg(LeggedRobotCfg):
    ...
    class init_state(LeggedRobotCfg.init_state):
        ...
        default_joint_angles = {
            'J_L0':       0.0,     # left  hip yaw
            'J_L1':       0.0,    # left  hip roll — slight abduction
            'J_L2':       0.0,     # left  hip pitch — slight flexion (+Y axis)
            'J_L3':       0.0,     # left  knee — bent (+Y axis, negative = flex)
            'J_L4_ankle': 0.0,     # left  ankle — dorsiflexion (-Y axis, positive = DF)
            'J_R0':       0.0,     # right hip yaw
            'J_R1':       0.0,    # right hip roll — slight abduction
            'J_R2':       0.0,     # right hip pitch — same flex (-Y axis, negative = flex)
            'J_R3':       0.0,     # right knee — bent (-Y axis, positive = flex)
            'J_R4_ankle': 0.0,    # right ankle — dorsiflexion (+Y axis, negative = DF)
        }
        ...
        root_pos_range = [
            [0., 0.],              # x
            [0., 0.],              # y
            [0.34, 0.37],          # z  (box foot bottom ~0.341 m; 4 mm clearance)
    ...
    class scaling(LeggedRobotCfg.scaling):
        ...
        dof_pos_target    = 1.
        commands          = 1.
        clip_actions      = 1.0            # BD joint limits are small (hip yaw ±0.3 rad)
...
class BDControllerRunnerCfg(LeggedRobotRunnerCfg):
    ...
    class policy(LeggedRobotRunnerCfg.policy):
        init_noise_std = 0.1           # small initial exploration noise; Xavier output dominates at 1.0
