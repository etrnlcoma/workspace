...
class BDControllerCfg(LeggedRobotCfg):
    ...
    class init_state(LeggedRobotCfg.init_state):
        ...
        pos = [0., 0., 0.33]          # x,y,z [m] — box foot bottom at ~0.341 m; 4 mm clearance
        rot = [0.0, 0.0, 0.0, 1.0]
        lin_vel = [0.0, 0.0, 0.0]
        ang_vel = [0.0, 0.0, 0.0]
    ...
    class commands(LeggedRobotCfg.commands):
        ...
        dstep_length = 0.5         # nominal step length [m]
        dstep_width  = 0.06         # nominal step width  [m]  ≈ hip width

        class ranges(LeggedRobotCfg.commands.ranges):
            sample_period = [35, 36]        # gait frequency ≈ 2.9 Hz
            dstep_width = [0.05, 0.07]      # [m]
