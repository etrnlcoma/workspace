...
class BDController(HumanoidController):
    ...
    def check_termination(self):
        ...
        term_contact = torch.norm(
            ...
            dim=-1)
        c1 = torch.any((term_contact > 1.), dim=1)
        ...
        c6 = torch.any(self.base_pos[:, 2:3] < 0.15, dim=1)

        self.terminated = c1 | c2 | c3 | c4 | c5 | c6

        # --- DEBUG (remove after diagnosis) ---
        if self.terminated.any() and self.episode_length_buf[0] <= 2:
            import sys
            all_cf = torch.norm(self.contact_forces[0], dim=-1)  # (num_bodies,)
            print(
                f"[TERM] ep_len={self.episode_length_buf[0].item():3d}"
                f"  contact={c1[0].item()}"
                f"  linvel={c2[0].item()} ({torch.norm(self.base_lin_vel[0]).item():.2f})"
                f"  angvel={c3[0].item()} ({torch.norm(self.base_ang_vel[0]).item():.2f})"
                f"  grav_x={c4[0].item()} ({self.projected_gravity[0,0].item():.3f})"
                f"  grav_y={c5[0].item()} ({self.projected_gravity[0,1].item():.3f})"
                f"  height={c6[0].item()} (z={self.base_pos[0,2].item():.3f})"
                f"\n  term_indices={self.termination_contact_indices.tolist()}"
                f"\n  all_contact_forces={[f'{v:.1f}' for v in all_cf.tolist()]}",
