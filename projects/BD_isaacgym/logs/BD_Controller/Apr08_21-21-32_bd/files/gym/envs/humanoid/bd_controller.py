...
class BDController(HumanoidController):
    ...
    def check_termination(self):
        ...
        term_contact = torch.norm(
            ...
            dim=-1)
        cond_contact  = torch.any((term_contact > 1.), dim=1)
        cond_lin_vel  = torch.any(torch.norm(self.base_lin_vel, dim=-1, keepdim=True) > 10., dim=1)
        cond_ang_vel  = torch.any(torch.norm(self.base_ang_vel, dim=-1, keepdim=True) > 5., dim=1)
        cond_grav_x   = torch.any(torch.abs(self.projected_gravity[:, 0:1]) > 0.7, dim=1)
        cond_grav_y   = torch.any(torch.abs(self.projected_gravity[:, 1:2]) > 0.7, dim=1)

        # --- DEBUG (remove after diagnosis) ---
        if self.common_step_counter < 5:
            env0 = 0
            print(f"[DBG step={self.common_step_counter}] ep_len={self.episode_length_buf[env0].item():.0f} "
                  f"z={self.base_pos[env0,2].item():.3f} "
                  f"contact={cond_contact[env0].item()} linvel={cond_lin_vel[env0].item()} "
                  f"angvel={cond_ang_vel[env0].item()} grav_x={cond_grav_x[env0].item()} grav_y={cond_grav_y[env0].item()}")
            print(f"  term_contact forces (env0): {term_contact[env0].tolist()}")
            print(f"  proj_grav (env0): {self.projected_gravity[env0].tolist()}")
            print(f"  base_lin_vel (env0): {self.base_lin_vel[env0].tolist()}")
            print(f"  base_ang_vel (env0): {self.base_ang_vel[env0].tolist()}")
        # --- END DEBUG ---

        self.terminated = cond_contact | cond_lin_vel | cond_ang_vel | cond_grav_x | cond_grav_y
