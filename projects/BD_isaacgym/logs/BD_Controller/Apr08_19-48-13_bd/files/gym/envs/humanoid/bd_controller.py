...
class BDController(HumanoidController):
    ...
    def check_termination(self):
        ...
        if self.terminated.any() and self.episode_length_buf[0] <= 2:
            import sys
            all_cf = torch.norm(self.contact_forces[0], dim=-1)
            body_pos = self.rigid_body_state[0, :, :3]  # (num_bodies, 3)
            print(
                f"[TERM] ep_len={self.episode_length_buf[0].item():3d}"
                f"  angvel={torch.norm(self.base_ang_vel[0]).item():.2f}"
                f"  z={self.base_pos[0,2].item():.3f}"
                f"\n  all_cf={[f'{v:.0f}' for v in all_cf.tolist()]}"
                f"\n  body_z={[f'{body_pos[i,2].item():.3f}' for i in range(len(all_cf))]}",
